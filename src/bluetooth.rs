...
#[cfg(not(any(target_os = "linux", target_os = "android", feature = "bluetooth-test")))]
...
pub struct BluetoothAdapter {
    ...
    #[cfg(not(any(target_os = "linux", target_os = "android", feature = "bluetooth", feature = "bluetooth-test")))]
...
impl BluetoothAdapter {
    ...
    #[cfg(all(target_os = "android", feature = "bluetooth"))]
    pub fn init() -> Result<BluetoothAdapter, Box<Error>> {
        let blurdroid_adapter = try!(BluetoothAdapterAndroid::adapter);
    ...
    #[cfg(feature = "bluetooth-test")]
    pub fn init() -> Result<BluetoothAdapter, Box<Error>> {
        Ok(BluetoothAdapter{adapter: FakeBluetoothAdapter::new_empty(String::from("FakeAdapter"))})
    }
    ...
    #[cfg(feature = "bluetooth-test")]
    fn get_mut_adapter(&mut self) -> &mut FakeBluetoothAdapter {
        Arc::get_mut(&mut self.adapter).unwrap()
    }
    ...
    pub fn get_devices(&self) -> Result<Vec<BluetoothDevice>, Box<Error>> {
        let device_list = try!(self.adapter.get_device_list());
...
impl BluetoothDiscoverySession {
    ...
    #[cfg(all(target_os = "android", feature = "bluetooth"))]
    pub fn create_session(adapter: BluetoothAdapter) -> Result<BluetoothDiscoverySession, Box<Error>> {
        let blurdroid_session = try!(BluetoothDiscoverySessionAndroid::create_session(adapter.adapter));
        Ok(BluetoothDiscoverySession{
            adapter: RefCell::new(adapter),
    ...
    #[cfg(not(any(target_os = "linux", target_os = "android", feature = "bluetooth", feature = "bluetooth-test")))]
    pub fn create_session(adapter: BluetoothAdapter) -> Result<BluetoothDiscoverySession, Box<Error>> {
        let empty_session = try!(BluetoothDiscoverySessionEmpty::create_session(adapter.adapter));
        Ok(BluetoothDiscoverySession{
            adapter: RefCell::new(adapter),
    ...
    #[cfg(feature = "bluetooth-test")]
    pub fn create_session(adapter: BluetoothAdapter) -> Result<BluetoothDiscoverySession, Box<Error>> {
        let test_session = try!(FakeBluetoothDiscoverySession::create_session(adapter.adapter.clone()));
        Ok(BluetoothDiscoverySession{
            adapter: RefCell::new(adapter),
...
impl BluetoothDevice {
    ...
    pub fn create_device(adapter: BluetoothAdapter, device: String) -> BluetoothDevice {
        BluetoothDevice{
            ...
            device: Arc::new(BluetoothDeviceAndroid::new(adapter.adapter, device.clone())),
        }
    }
    ...
    #[cfg(all(feature = "bluetooth-test"))]
    pub fn create_device(adapter: BluetoothAdapter, device_id: String) -> BluetoothDevice {
        BluetoothDevice {
            adapter: RefCell::new(adapter.clone()),
            device: FakeBluetoothDevice::new_empty(adapter.adapter, device_id),
        }
    }
    ...
    pub fn set_alias(&self, value: String) -> Result<(), Box<Error>> {
        Arc::make_mut(&mut self.get_device()).set_alias(value)
    }
    ...
    pub fn connect(&self) -> Result<(), Box<Error>> {
        Arc::make_mut(&mut self.get_device()).connect()
    }

    pub fn disconnect(&self) -> Result<(), Box<Error>> {
        Arc::make_mut(&mut self.get_device()).disconnect()
    }

    pub fn connect_profile(&self, uuid: String) -> Result<(), Box<Error>> {
        Arc::make_mut(&mut self.get_device()).connect_profile(uuid)
    }

    pub fn disconnect_profile(&self, uuid: String) -> Result<(), Box<Error>> {
         Arc::make_mut(&mut self.get_device()).disconnect_profile(uuid)
    }

    pub fn pair(&self) -> Result<(), Box<Error>> {
         Arc::make_mut(&mut self.get_device()).pair()
    }

    pub fn cancel_pairing(&self) -> Result<(), Box<Error>> {
         Arc::make_mut(&mut self.get_device()).cancel_pairing()
    }
}
...
impl BluetoothGATTCharacteristic {
    ...
    pub fn write_value(&self, value: Vec<u8>) -> Result<(), Box<Error>> {
        Arc::make_mut(&mut self.get_gatt_characteristic()).write_value(value)
    }

    pub fn start_notify(&self) -> Result<(), Box<Error>> {
         Arc::make_mut(&mut self.get_gatt_characteristic()).start_notify()
    }

    pub fn stop_notify(&self) -> Result<(), Box<Error>> {
         Arc::make_mut(&mut self.get_gatt_characteristic()).stop_notify()
    }
}
...
impl BluetoothGATTDescriptor {
    ...
    pub fn write_value(&self, value: Vec<u8>) -> Result<(), Box<Error>> {
        match Arc::get_mut(&mut self.get_gatt_descriptor()) {
            Some(descriptor) => descriptor.write_value(value),
            None => Err(Box::from("Could not write the descriptor.")),
        }
    }
}
